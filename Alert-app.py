...
st.markdown("Upload your disease surveillance data file (Excel or CSV, max 200MB). Ensure it includes columns: `periodname` (e.g., 'Week 38'), `orgunitlevel1`–`orgunitlevel6`, `organisationunitname`, and disease names (e.g., 'Acute Diarrhea (New Cases)').")
    ...
    with st.spinner("Processing your file..."):
        ...
        try:
            if uploaded_file.name.endswith('.xlsx'):
                ...
                st.write("Excel file loaded. Checking columns...")
                df.columns = df.columns.str.strip()
                
                # Verify required columns
                required_cols = ['periodname', 'organisationunitname']
                missing_cols = [col for col in required_cols if col not in df.columns]
                if missing_cols:
                    st.error(f"❌ Missing required columns: {', '.join(missing_cols)}. Please check your file.")
                    st.stop()
                
                # Extract Epi Week Number
                df['Epi Week Number'] = df['periodname'].str.extract(r'Week (\d+)', expand=False)
                if df['Epi Week Number'].isna().any():
                    st.error("❌ Some 'periodname' values do not match the expected format (e.g., 'Week 38'). Please ensure all periodname entries are valid.")
                    st.stop()
                df['Epi Week Number'] = df['Epi Week Number'].astype(int)
                ...
                if not disease_cols:
                    st.error("❌ No disease columns found. Please ensure your file includes columns with disease case counts.")
                    st.stop()
                
                st.write("Converting data to long format...")
                long_df = pd.melt(
                    df,
                ...
                missing_cols = [col for col in expected_cols if col not in long_df.columns]
                if missing_cols:
                    st.error(f"❌ CSV missing required columns: {', '.join(missing_cols)}. Please check your file.")
                    st.stop()
            ...
            st.error("Please check your file format (Excel/CSV) and ensure it contains the required columns. Contact support if the issue persists.")
            st.stop()
